...
function Container() {
    ...
    chrome.storage.sync.get(function (items) {
        ...
        that.getElement().style.display = items.display || 'block';
    });
    /**
    ...
    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        if (request.message == 'display') {
            if (style.display === 'block') {
                ...
                that.show();
                chrome.storage.sync.set({display: 'block'}, function () {

                });
            }
        }

        if (request.message == 'opacity') {
            chrome.storage.sync.get(function (items) {
                that.arrow_up.setOpacity(items.opacity || 0.2);
                that.arrow_down.setOpacity(items.opacity || 0.2);
            });
        }

        if(request.message == 'src') {
            chrome.storage.sync.get(function (items) {
                that.arrow_up.setSrc(items.src);
                that.arrow_down.setSrc(items.src);
            });
        }

        if(request.message == 'position') {
            chrome.storage.sync.get(function (items) {
                that.setPosition(items.position);
            });
        }

        if(request.message == 'speed') {
            chrome.storage.sync.get(function (items) {
                that.arrow_down.setSpeed(items.speed);
                that.arrow_up.setSpeed(items.speed);
            });
        }
